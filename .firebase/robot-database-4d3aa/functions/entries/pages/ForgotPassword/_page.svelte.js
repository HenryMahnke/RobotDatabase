...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  return `${validate_component(Blockquote, "Blockquote").$$render(
    $$result,
    {
      border: true,
      bg: true,
    ...
    {
      default: () => {
        return `${validate_component(P, "P").$$render($$result, { size: "xl", height: "relaxed" }, {}, {
          default: () => {
            return `Because we can only have a single account per team in order to prevent malicious behavior, if the email is the same the recovery process will be much simpler, otherwise, you will have to reauthenticate your account.`;
          }
        })}`;
      }
    }
  )} ${validate_component(Card, "Card").$$render($$result, { class: "w-full max-w-md m-auto" }, {}, {
    default: () => {
      return `<form class="flex flex-col space-y-6" action="/"><h3 class="text-xl font-medium text-gray-900 dark:text-white" data-svelte-h="svelte-1cvozd8">Recover an Account</h3> ${validate_component(Label, "Label").$$render($$result, { class: "space-y-2" }, {}, {
        default: () => {
